...
export const generateMeal = async(filters) => {
    try{
        let mealRecommendation = await ai.sendMessage(`I just want the mealName, 1 sentence concise description (description), nutritional values with unit(nutritionalValues), and the prepTime based on this filters: 
...
export const generateSmartMeal = async(mealList) => {
    try{
        let mealRecommendation = await ai.sendMessage(`Recommend a new meal based on the list. AVOID recommending a meal with the same mealName in the list, this is important so please remember this. 
                I just want the mealName, 1 sentence concise description(description), nutritional values with unit(nutritionalValues), reason why you recommended it(reason), and the prepTime based on this existing list: 
